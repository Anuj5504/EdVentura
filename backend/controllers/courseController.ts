...
export const addQuestion = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        await NotificationModel.create({
            user: req.user?._id,
            title: "New Question!",
            message: `You have new question in ${content.title}`
        });
    ...
    try {
        ...
        if (req.user?._id.toString() !== question.user._id.toString()) {
            ...
            await NotificationModel.create({
                ...
                message: `You have new Question reply in ${content.title}`
            });
...
export const addReview = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const courseExist = courseList?.some((course: any) => course._id.toString() === courseId);
        ...
        if (course) {
            ...
            course.ratings = avgRating / course.review.length;
        }
        await course?.save();
...
export const addReply = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { reply, courseId, reviewId } = req.body as IReply;
        ...
        const review = course.review.find((item: any) => item._id.equals(reviewId));
...
export const getAllCoursesAuth = catchAsyncError(async (req: Request, res: Response, next: NextFunction) => { 
    try {
